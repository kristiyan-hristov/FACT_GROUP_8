...
from tgnnexplainer.src.dataset.tg_dataset import load_tg_dataset, load_explain_idx
from tgnnexplainer.src.dataset.utils_dataset import construct_tgat_neighbor_finder

from tgnnexplainer.src.models.ext.tgat.module import TGAN
from tgnnexplainer.src.models.ext.tgn.model.tgn import TGN
from tgnnexplainer.src.models.ext.tgn.utils.data_processing import compute_time_statistics
from tgnnexplainer.src import ROOT_DIR
...
def pipeline(config: DictConfig):
    ...
    config.models.ckpt_path = str(ROOT_DIR/'src'/'models'/'checkpoints'/f'{config.models.model_name}_{config.datasets.dataset_name}_best.pth')

    # dataset config
    config.datasets.dataset_path = str(ROOT_DIR/'src'/'dataset'/'data'/f'{config.datasets.dataset_name}.csv')
    config.datasets.explain_idx_filepath = str(ROOT_DIR/'src'/'dataset'/'explain_index'/f'{config.datasets.explain_idx_filename}.csv')

    # explainer config
    config.explainers.param = config.explainers.param[config.datasets.dataset_name]
    config.explainers.results_dir = str(ROOT_DIR.parent/'benchmarks'/'results')
    config.explainers.mcts_saved_dir = str(ROOT_DIR/'src'/'saved_mcts_results')
    config.explainers.explainer_ckpt_dir = str(ROOT_DIR/'src'/'explainer_ckpts')
    ...
    if config.explainers.explainer_name == 'tgnnexplainer': # DONE: test this 'use_pg_explainer'
        from tgnnexplainer.src.method.tgnnexplainer import TGNNExplainer
        from tgnnexplainer.src.method.other_baselines_tg import PGExplainerExt
        ...
        from tgnnexplainer.src.method.attn_explainer_tg import AttnExplainerTG
        ...
        from tgnnexplainer.src.method.other_baselines_tg import PBOneExplainerTG
    ...
    if config.explainers.explainer_name == 'tgnnexplainer':
        from tgnnexplainer.src.evaluation.metrics_tg import EvaluatorMCTSTG
        ...
        from tgnnexplainer.src.evaluation.metrics_tg import EvaluatorAttenTG
